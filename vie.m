...
% File: vie.m
%
% Goal:  Define the analytical expressions of the kernel, the right-hand 
%        side term and the weight function u of the weighted space, 
%        for the given VIE
%
% Use: [g] = vie()
%
...
g{2} = @(t) t-1/2*(-2+(exp(-t/2).*(10+t))./(5+t)+exp(5+t/2).*(12+t) ...
    .*(real(-expint(5+t))-real(-expint((5+t/2))))); % G

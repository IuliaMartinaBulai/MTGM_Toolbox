...
% File: VieSolve.m
%
% Goal: Compute the required solution of the Volterra Integral Equation,
%       the dimension of the solved linear system and its condition number
%
% Use: [M,N,j,C] = VieSolve(kind,T0,t,m,type_OB,varargin)
%
% Input: kind - can assume values:
%             - 0 - to solve a Volterra Integral Equation
%             - 1 - to compute the cumulative number of metastases and the
%                   total metastatic mass for the 1D metastatic tumor
%                   growth model assuming that the growth laws for the
%                   primary and secondary tumor are known analytically;
%             - 2 - to compute the cumulative number of metastases and the
%                   total metastatic mass for the 1D metastatic tumor
%                   growth model solving numerically the differential
%                   problem describing the growth of the tumor;
...
if kind == 0
    ...
    % compute M and N, only for the 2D non-autonomous case Fpsi must be
    % called otherwise use Fbeta
elseif kind == 1
    if type_OB == 1

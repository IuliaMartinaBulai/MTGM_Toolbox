...
%          ode23t.m, odefun1Dm.m, tumorTreatmentParam.m, odefun2Dm.m
%
...
elseif kind == 1
    [~,~,be_m,vm] = tumorGrowthFunctions(varargin{:});
...
elseif kind == 2
    [g,y0_m,~,param,~,options] = tumorGrowthFunctionsOdeSolve(varargin{:});
    be_p = g{1};
    be_m = g{2};
    m = length(x1);
    n = length(x2);
    ...
    parfor i = 1:n
        for k = 1:m
            if (x2(i)-x1(k)) > 0
                [~,sol1] = ode23t(@(tt,yy)odefun1Dm (tt, yy, param), ...
                    [0 x2(i)-x1(k)],y0_m,options);
...
elseif kind == 3
    [g,~,y0_m,param,~,options] = tumorTreatmentParam(varargin{:});
    be_p = g{1};
    be_m = g{2};
    m = length(x1);
    n = length(x2);

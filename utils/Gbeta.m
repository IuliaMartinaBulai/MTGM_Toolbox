...
%
% Output: gtbeta - array of the right-hand side of the VIE at the
%                  evaluation points t
%
% Recalls: vie.m, tumorGrowthFunctions.m, tumorGrowthFunctionsOdeSolve.m,
%          ode23t.m, odefun1Dm .m, odefun1Dp.m tumorTreatmentParam.m,
%          odefun2Dm.m, odefun2Dp.m
%
...
%--------------------------------------------------------------------------
function [gtbeta] = Gbeta(kind,t,w1,E1,type_OB,varargin)
...
if kind == 0
    ...
    g2 = g{2};
    gtbeta = g2(t);
    % kind = 1: Cumulative number of metastases and total metastatic mass
    % for the 1D metastatic tumor growth model
elseif kind == 1
...
elseif kind == 2
    [g,y0_m,y0_p,param,vbar,options] = tumorGrowthFunctionsOdeSolve(varargin{:});
    ...
    if type_OB == 1
        ...
        parfor i = 1:m
            y = t(i);
            r = 1;
            [~,sol1] = ode45(@(tt,yy)odefun1Dm (tt,yy,param),...
                [0 t(i) - Y(r,i)],y0_m,options);
            [~,sol2] = ode45(@(tt,yy)odefun1Dp(tt,yy,param),...
                [0 Y(r,i)],y0_p,options);
    ...
    elseif type_OB == 2
        ...
        parfor i = 1:m
            ...
            [~,sol1] = ode45(@(tt,yy)odefun1Dm (tt,yy,param),...
                [0 t(i) - Y(r,i)],y0_m,options);
            [~,sol2] = ode45(@(tt,yy)odefun1Dp(tt,yy, param),...
                [0 Y(r,i)],y0_p,options); %Y(r,i)
            g = w1(r)*y*characteristic(sol1(end,1),vbar)*be_p(sol2(end,1));
            while (r < length(w1)) && (abs(g) > 0.5e-25)
                gtbeta(i) = gtbeta(i)+g;
                r = r+1;
                [~,sol1] = ode45(@(tt,yy)odefun1Dm (tt,yy,param),...
    ...
    % kind = 3: Cumulative number of metastases and total metastatic mass 
    %           for the 2D metastatic tumor growth model with treatment 
    %           solving the 2D ODE non-autonomous system
elseif kind == 3
    Y = t.*E1';
    ...
    parfor i = 1:m
        ...
        if y ~= 0
            r = 1;
            [~,sol1] = ode23t(@(tt,yy)odefun2Dm(tt, yy, param),...
                [Y(r,i) t(i)],y0_m,options);
            [~,sol2] = ode23t(@(tt,yy)odefun2Dp(tt, yy, param),...
                [0 Y(r,i)],y0_p,options);
    ...
    % kind = 4: Cumulative number of metastases and total metastatic mass 
    %           for the 2D metastatic tumor growth model solving the 2D ODE
    %           autonomous system.
elseif kind == 4
    ...
    if type_OB == 1
        ...
        parfor i = 1:m
            ...
            r = 1;
            [~,sol1] = ode23t(@(tt,yy)odefun2Dm(tt,yy,param),...
                [0 Y(r,i)],y0_m,options);
            [~,sol2]=ode23t(@(tt,yy)odefun2Dp(tt,yy, param),...
                [0 t(i)- Y(r,i)],y0_p,options);
            g1 = w1(r)*y*sol1(end,1)*be_p(sol2(end,1));
            while (r< length(w1)) && (abs(g1)>0.5e-25)
                gtbeta(i) = gtbeta(i)+g1;
                r = r+1;
    ...
    elseif type_OB == 2
        ...
        parfor i = 1:m
            ...
            [~,sol2] = ode23t(@(tt,yy)odefun2Dp(tt,yy, param),...
                [0 t(i)- Y(r,i)],y0_p,options);
            g1 = w1(r)*y*characteristic(sol1(end,1),vbar)*...
                be_p(sol2(end,1));
            while (r < length(w1)) && (abs(g1) > 0.5e-25)
                gtbeta(i) = gtbeta(i)+g1;
                r = r+1;
                ...
                [~,sol2] = ode23t(@(tt,yy)odefun2Dp(tt,yy, param),...
                    [0 t(i)- Y(r,i)],y0_p,options);
                g1 = w1(r)*y*characteristic(sol1(end,1),vbar)*be_p(sol2(end,1));
            end
        end
    end
end
